...
async function setupTokenHandler(root) {
  ...
  const {
    ConsoleOAuthFlow
  } = await import("./ConsoleOAuthFlow-YYFGPUNP.mjs");
...
var DoctorLazy = react_default.lazy(() => import("./Doctor-G3NN5IJV.mjs").then((m) => ({
  default: m.Doctor
})));
...
async function installHandler(target, options) {
  ...
  const {
    install
  } = await import("./install-BH6UC52O.mjs");
  await new Promise((resolve) => {
    const args = [];
