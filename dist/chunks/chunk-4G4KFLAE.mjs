...
async function createBridgeSession({
  ...
  const { getClaudeAIOAuthTokens } = await import("./auth-GOR7XA2X.mjs");
  const { getOrganizationUUID } = await import("./client-BTLADLCF.mjs");
  const { getOauthConfig } = await import("./oauth-Y6DQDGXF.mjs");
  const { getOAuthHeaders } = await import("./api-P5UKGRTV.mjs");
  const { parseGitHubRepository } = await import("./detectRepository-6HDSAE4F.mjs");
  const { getDefaultBranch } = await import("./git-MPST6L6M.mjs");
  const { getMainLoopModel } = await import("./model-2EAQG4BA.mjs");
