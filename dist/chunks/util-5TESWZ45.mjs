...
async function setupTokenHandler(root) {
  ...
  const {
    ConsoleOAuthFlow
  } = await import("./ConsoleOAuthFlow-XQPRRFVE.mjs");
...
var DoctorLazy = react_default.lazy(() => import("./Doctor-EEJK6YYS.mjs").then((m) => ({
  default: m.Doctor
})));
...
async function installHandler(target, options) {
  const {
    setup
  } = await import("./setup-MOTT5PYB.mjs");
  await setup(cwd(), "default", false, false, void 0, false);
  const {
    install
  } = await import("./install-N55ENLW3.mjs");
  await new Promise((resolve) => {
    const args = [];
