...
async function setupTokenHandler(root) {
  ...
  const {
    ConsoleOAuthFlow
  } = await import("./ConsoleOAuthFlow-PJ2YM4NZ.mjs");
...
var DoctorLazy = react_default.lazy(() => import("./Doctor-7JYVOJ3E.mjs").then((m) => ({
  default: m.Doctor
})));
...
async function installHandler(target, options) {
  ...
  const {
    install
  } = await import("./install-CQPCKUWN.mjs");
  await new Promise((resolve) => {
    const args = [];
