...
async function createBridgeSession({
  ...
  const { getClaudeAIOAuthTokens } = await import("./auth-XO5IY6CP.mjs");
  const { getOrganizationUUID } = await import("./client-LOU7ORMG.mjs");
  const { getOauthConfig } = await import("./oauth-Y6DQDGXF.mjs");
  const { getOAuthHeaders } = await import("./api-HXJBMWTX.mjs");
  ...
  const { getMainLoopModel } = await import("./model-5Z5DMYCH.mjs");
