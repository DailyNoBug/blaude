...
async function setupTokenHandler(root) {
  ...
  const {
    ConsoleOAuthFlow
  } = await import("./ConsoleOAuthFlow-B5OOGRLK.mjs");
...
var DoctorLazy = react_default.lazy(() => import("./Doctor-TP6XQRB2.mjs").then((m) => ({
  default: m.Doctor
})));
...
async function installHandler(target, options) {
  const {
    setup
  } = await import("./setup-7X777VCC.mjs");
  await setup(cwd(), "default", false, false, void 0, false);
  const {
    install
  } = await import("./install-QUMFRVCQ.mjs");
  await new Promise((resolve) => {
    const args = [];
