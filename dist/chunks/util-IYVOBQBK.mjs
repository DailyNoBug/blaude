...
async function setupTokenHandler(root) {
  ...
  const {
    ConsoleOAuthFlow
  } = await import("./ConsoleOAuthFlow-XEHFPHHX.mjs");
...
var DoctorLazy = react_default.lazy(() => import("./Doctor-JKSGWAS3.mjs").then((m) => ({
  default: m.Doctor
})));
...
async function installHandler(target, options) {
  const {
    setup
  } = await import("./setup-TPPDV2AE.mjs");
  await setup(cwd(), "default", false, false, void 0, false);
  const {
    install
  } = await import("./install-J6YEQWR4.mjs");
  await new Promise((resolve) => {
    const args = [];
