...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another Blaude session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'Blaude is using your computer · press Esc to stop' : 'Blaude is using your computer · press Ctrl+C to stop',
