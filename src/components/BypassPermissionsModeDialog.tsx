...
export function BypassPermissionsModeDialog(t0) {
  ...
  let t5;
  if ($[5] !== onChange) {
    t5 = <Dialog title="WARNING: Blaude running in Bypass Permissions mode" color="error" onCancel={handleEscape}>{t3}<Select options={t4} onChange={value_0 => onChange(value_0 as 'accept' | 'decline')} /></Dialog>;
    $[5] = onChange;
    $[6] = t5;
  } else {
    t5 = $[6];
