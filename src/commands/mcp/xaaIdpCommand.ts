...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "Blaude's client_id at the IdP")
    .option(
      '--client-secret',
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          "Error: no XAA IdP connection. Run 'blaude mcp xaa setup' first.",
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : "no — run 'blaude mcp xaa login'"}\n`,
      )
      cliOk()
    })
