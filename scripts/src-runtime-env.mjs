...
export async function installSrcRuntimeGlobals(root = process.cwd()) {
...
export function applySrcRuntimeDefaults() {
  ...
  const current = String(process.env.BLAUDE_SRC_FEATURES || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
